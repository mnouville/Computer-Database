...
public class Controller {
	...
	public void addComputer() throws SQLException {
		...
		try  {  
			...
			if ( this.serviceCompany.companyExist(company_id) )	{
				Computer c = new Computer(this.serviceComputer.getMaxId(),name,ts1,ts2,this.daoFactory.getCompanyDao().getCompany(company_id));
				...
				LOG.error("The following Company Does not exist : " + company_id);
	...
	public void updateComputer() throws SQLException {
		...
		try  {  
			...
			try  {  
				...
				if ( this.serviceCompany.companyExist(company_id) )	{
					Computer c = new Computer(i,name,ts1,ts2,this.daoFactory.getCompanyDao().getCompany(company_id));
					//check if all informations are valid
					if ( c.validComputer() ) {
	...
	public void getComputers() throws SQLException {
	    ...
	    TextTable table = new TextTable(new String[] {"ID","Name","Introduced","Discontinued","Company_Id"},data);
	    this.getView().printTableDatabase(table);
